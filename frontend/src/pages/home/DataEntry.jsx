      ...
      api
        ...
        .then((res) => {
          ...
          const diseases = res.data.diseases.map((disease) => ({
            value: disease.id,
            label: disease.diseaseName + "(" + disease.diseaseCode + ")",
    ...
    api
      ...
      .then((res) => {
        if (res.data.last_opd) {
          initialPatentData["serialNumber"] = Number(res.data.last_opd) + 1;
        }
      })
      .catch((err) => console.log(err));
  }, []);
  ...
  const handleSubmitToDatabase = () => {
    ...
    diagnosisArray.forEach((arr, index) => {
      formDiagnosisData.forEach((d) => {
        if (d.required == true && (!arr[d.name] || arr[d.name].length == 0)) {
          showAlert(
            d.label +
      ...
      if (arr["diagnosis"].length > 1) {
        showAlert("Multilple diagnosis detected on row #" + (index + 1));
        setSubmitNow(false);
        return;
      }
      ...
      if (arr["result"] && !arr["test"]) {
        showAlert(
          "Result without test, Please make corrections row #" + (index + 1),
        );
      ...
      if (!arr["result"] && arr["test"]) {
        showAlert(
          "Test without Result, Please make corrections row #" + (index + 1),
        );
    ...
    if (submitNow) {
      ...
      try {
        createRecords(data, "patients/create", false).then((res) => {
          ...
          patientData["serialNumber"] = Number(patientData["serialNumber"]) + 1;
          patientData["gender"] = "";
          patientData["village"] = [];
          patientData["age"] = "";
          setDiagnosisArray([]);
      ...
      <div className="review-container" container>
        ...
        {submit && (
          <div className="review-container">
            ...
            <div className="review-values">
              {formPatientData.map((d, index) => {
                return (
                  <p
                    key={index}
                    onClick={() => handlePatientReviewClick(index)}
                  >
                    {d.label}:{" "}
            ...
            <div className="buttons">
              <button onClick={handleReviewBackButtonClick}>Back</button>
              <button onClick={handleSubmitToDatabase}>Submit</button>
            </div>
          </div>
        )}
      </div>
    ...
    <table>
      ...
      <tbody>
        ...
        {diagnosisArray.length === 0 && (
          <tr>
            <td colSpan={5}>
              <p>No "Diagnosis and Treatment" Data recorded yet!</p>
            </td>
          </tr>
        )}
      </tbody>
