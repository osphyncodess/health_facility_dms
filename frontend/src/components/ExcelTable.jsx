...
const ExcelTable = ({
  ...
  setShowVillageForm,
  setWhat,
  villageCollect = false,
  treatmentCollect = false,
  diseaseCollect = false,
  setSerialNumberKey,
  serialNumberKey,
  setLastSerialUrl,
  lastSerialUrl,
  ...
  const createEmptyRow = () => {
    ...
    columns.forEach((c) => {
      if (c.key === "pairs") {
        data[c.key] = [{ condition: "", treatment: [] }];
      } else {
        if (c.default) {
          data[c.key] = c.default;
        } else {
          data[c.key] = "";
        }
      }
  ...
  const [data, setData] = useState(Array.from({ length: 20 }, createEmptyRow));
  ...
  useEffect(() => {
    api
      .get(lastSerialUrl)
      .then((res) => {
        updateCell(0, serialNumberKey, Number(res.data.last_opd) + 1);
