...
const ExcelTable = ({
    ...
    const removePairTreatment = (row, i, value) => {
        const updated = [...data];
        updated[row].pairs[i].treatment = updated[row].pairs[
            i
        ].treatment.filter(t => t !== value);
        setData(updated);
    };
    ...
    const handlePairKeyDown = (e, row, col, pairIndex, type, options) => {
        ...
        switch (e.key) {
            case "Enter":
                ...
                if (type === "condition") {
                    const condition =
                        refs.current[getPairKey(row, col, pairIndex, type)];
                    ...
                    if (condition.value) {
                        ...
                        refs.current[
                            getPairKey(row, col, pairIndex, "treatment")
                        ]?.focus();
                    }
                }
                ...
                if (type === "treatment") {
                    ...
                    if (treatment.value) {
                        ...
                        if (pairsCount === pairIndex) {
                            refs.current[
                                getPairKey(row + 1, col, 0, "condition")
                            ]?.focus();
                        } else {
                            refs.current[
                            ...
                            {data.map((row, r) => (
                                <tr key={r}>
                                    ...
                                    {columns.map((col, c) => {
                                        ...
                                        if (col.type === "pair") {
                                            const conditionsAll =
                                                col.options.conditions;
                                            const treatmentsAll =
                                                col.options.treatments;
